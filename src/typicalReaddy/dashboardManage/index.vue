<template>
  <div class="dashboard-wrap">
    ...
    <div class="table">
            ...
            <template v-else-if="column.key === 'status'">
              <span @click.stop @mousedown.stop>
                <a-switch
                  ...
                  @update:checked="(val: any) => (statusMap[record.key] = val)"
                />
              </span>
            </template>
            <template v-else-if="column.key === 'operation'">
              <a-button class="link" :disabled="record.disabled" type="link">详情</a-button>
              <a-button class="link" :disabled="record.disabled" type="link">删除</a-button>
      ...
      <a-modal v-model:open="dialogVisible" title="导入" @ok="dialogVisible = false" @cancel="dialogVisible = false">
        <div>
          <a-upload-dragger
          ...
          >
            ...
            <p class="explain-text"><span class="click-btn">点击上传</span>&nbsp;&nbsp;/ &nbsp;&nbsp;拖拽到此区域</p>
          </a-upload-dragger>
          <p class="explain">支持格式文件</p>
        </div>
      </a-modal>

      <a-modal v-model:open="dialogMoveVisible" title="移动到" @ok="handleMoveConfirm" @cancel="handleMoveCancel">
        <a-form ref="ruleForm" :model="form" layout="horizontal">
          <a-form-item label="挂载分组" name="group">
            <a-select v-model:value="form.group" placeholder="挂载分组">

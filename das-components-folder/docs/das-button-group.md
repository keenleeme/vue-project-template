<ComponentInfo developer="王银" date="2025-03-28" />

# das-button-group 按钮组

## 组件说明
ButtonGroup 是一个灵活的按钮组组件，用于展示一组相关的操作按钮。它支持多种布局方式和使用场景，能够满足不同业务场景下的按钮操作需求。组件提供了丰富的配置选项和事件，可以轻松实现各种复杂的按钮交互，支持一下特性：
...
展示表格行操作场景下的按钮组用法，纯文字按钮、带外链按钮：可跳出平台、可折叠按钮：同类操作可折叠或超出数量限制的按钮折叠到更多中。
...
### 3. 如何处理按钮的权限控制？
组件提供了多种方式来控制按钮的权限：
- 通过 disabled 属性控制整个按钮组或单个按钮的禁用状态
- 对于表格操作按钮，可以使用 needSelection 属性结合 hasSelection 来控制按钮是否可用
- 对于树形按钮，可以通过 treeButtons 中的 disabled 属性控制单个按钮的禁用状态

### 4. 如何实现按钮的加载状态？
组件支持两种级别的加载状态控制：
- 通过 loading 属性控制整个按钮组的加载状态
- 通过配置项（如 TreeButton、TableButton）中的 loading 属性控制单个按钮的加载状态
